// ==UserScript==
// @name         500px 图片选择器 (v1.0)
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  500px 图片批量选择下载。悬停显示选中按钮，S 键快捷选中，支持 ZIP 打包。
// @author       You & Claude
// @match        https://500px.com/*
// @match        https://www.500px.com/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_xmlhttpRequest
// @grant        GM_download
// @connect      drscdn.500px.org
// @connect      *.500px.org
// @connect      *
...
(function () {
    ...
    // === 配置 ===
    const ACCENT = '#0078FF';
    const ACCENT_RGB = '0,120,255';
    const SK = { sel: 'px5_selected_v1', prefix: 'px5_prefix', dock: 'px5_dock_collapsed' };

    // === 状态 ===
    let selected = new Map();
    let prefix = load(SK.prefix, '500px');
    let collapsed = false;
    let mx = -1, my = -1;
    let busy = false;
    let raf = null;
    ...
    function photoId(img) {
        // 从 URL 提取 photo ID: /photo/{id}/...
        const m = (img.src || '').match(/\/photo\/(\d+)\//);
        if (m) return m[1];
        // 从父级链接提取
        const a = img.closest('a[href*="/photo/"]');
        if (a) { const m2 = a.href.match(/\/photo\/(\d+)/); if (m2) return m2[1]; }
        // 详情页 URL
        const m3 = location.href.match(/\/photo\/(\d+)/);
        if (m3) return m3[1];
        return null;
    }
    ...
    function imgKey(img) {
        ...
        if (!url) return null;
        const pid = photoId(img);
        const h = hash(url).slice(0, 8);
        const id = pid ? `px5-${pid}` : `px5-${h}`;
        return { id, url };
    }
    ...
    function setup() {
        ...
        for (const img of imgs) {
            // 宿主：<a> 父级或 img 的直接父级
            const host = img.closest('a[href*="/photo/"]') || img.parentElement;
            if (!host) continue;
...
.ps-r:hover:not(:disabled) { background: #0066dd !important; }
...
`;
    ...
    function setupEvents() {
        ...
        // SPA 导航
        window.addEventListener('popstate', () => setTimeout(sched, 300));
        const oPush = history.pushState;
        history.pushState = function (...a) { oPush.apply(this, a); setTimeout(sched, 300); };
        const oRepl = history.replaceState;
        history.replaceState = function (...a) { oRepl.apply(this, a); setTimeout(sched, 300); };
    ...
    function imgAt(x, y) {
        for (const el of document.elementsFromPoint(x, y)) {
            ...
            // 宿主容器（<a> 等覆盖层）→ 找其中的 img
            if (el.classList.contains('ps-img-host')) {
                const img = el.querySelector('img');
                if (img && isContent(img)) return img;
            }
            // 宿主内部的非 img 子元素（渐变遮罩、信息层等）
            const host = el.closest('.ps-img-host');
            if (host) {
        ...
        return new Promise((ok, no) => {
            if (typeof GM_download === 'function') {
                GM_download({
                    ...
                    onload: ok,
                    onerror(e) {
                        console.warn('[500px] GM_download 失败, 回退 fetch:', e);
    ...
    function linksModal() {
        const m = showModal(`<h3>链接打包</h3><p>粘贴图片链接，一行一个</p><textarea placeholder="https://drscdn.500px.org/..."></textarea>`);
    ...
    function init() {
        ...
        save(SK.dock, false);
        console.log(`[500px Selector] v1.0 ready, ${selected.size} saved, ${contentImgs().length} images found`);
    }
