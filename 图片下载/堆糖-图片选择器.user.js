...
// @version      1.0
// @description  堆糖图片批量选择下载。悬停显示选中按钮，S 键快捷选中，支持 ZIP 打包。
// @author       You & Claude
// @match        https://www.duitang.com/*
// @match        https://duitang.com/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_xmlhttpRequest
// @grant        GM_download
// @connect      dtstatic.com
// @connect      c-ssl.dtstatic.com
// @connect      a-ssl.dtstatic.com
// @connect      *
...
(function () {
    ...
    // === 配置 ===
    const ACCENT = '#FF5777';
    const ACCENT_RGB = '255,87,119';
    const SK = { sel: 'dt_selected_v1', prefix: 'dt_prefix', dock: 'dt_dock_collapsed' };

    // === 状态 ===
    let selected = new Map();
    let prefix = load(SK.prefix, 'duitang');
    let collapsed = false;
    let mx = -1, my = -1;
    let busy = false;
    let raf = null;
    ...
    function origUrl(url) {
        if (!url) return '';
        // 移除缩略图处理指令 .thumb.300_300_c 或 .thumb.400_0
        let u = url.replace(/\.thumb\.\d+_\d+(_c)?/, '');
        // 移除 CDN webp 转换后缀 _webp
        u = u.replace(/_webp$/, '');
        return u;
    }
    function norm(url) {
    ...
    function bestUrl(img) {
        // 详情页：.img-out 的 href 是原图
        const imgOut = img.closest('.img-out');
        if (imgOut && imgOut.href) return norm(origUrl(imgOut.href));
        // data-src 优先
        const ds = img.getAttribute('data-src');
        const src = img.currentSrc || img.src || '';
        return norm(origUrl(ds || src));
    }
    function isContent(img) {
        ...
        if (/avatar|icon|logo/.test(cls)) return false;
        // 排除小图标（收藏按钮图标等）
        const id = (img.id || '').toLowerCase();
        if (/avatar|icon/.test(id)) return false;
        // 尺寸过滤
        const r = img.getBoundingClientRect();
        const dw = +img.getAttribute('data-width') || 0;
        const dh = +img.getAttribute('data-height') || 0;
        ...
        if (r.width > 0 && r.height > 0 && r.width * r.height < 3000) return false;
        // 必须有可用 URL
    ...
    function blogId(img) {
        // 从卡片链接提取 blog ID
        const a = img.closest('a[href*="/blog/"]');
        if (a) { const m = a.href.match(/[?&]id=(\d+)/); if (m) return m[1]; }
        // 详情页 URL
        const m = location.href.match(/\/blog\/\?id=(\d+)/);
        if (m) return m[1];
        return null;
    }
    ...
    function imgKey(img) {
        ...
        if (!url) return null;
        const bid = blogId(img);
        const h = hash(url).slice(0, 8);
        const id = bid ? `dt-${bid}-${h}` : `dt-${h}`;
        return { id, url };
    }
    ...
    function setup() {
        ...
        for (const img of imgs) {
            // 宿主：图片所在的卡片或直接父级
            const host = img.closest('.woo-pcont, .mbpho, .de-img, [class*="pcont"]') || img.parentElement;
            if (!host) continue;
...
.ps-r:hover:not(:disabled) { background: #e64d6a !important; }
...
#ps-dock .ps-pill:hover { transform: scale(1.08) !important; }
...
`;
    ...
    function linksModal() {
        const m = showModal(`<h3>链接打包</h3><p>粘贴图片链接，一行一个</p><textarea placeholder="https://c-ssl.dtstatic.com/..."></textarea>`);
        const ta = m.querySelector('textarea');
        const ok = document.createElement('button');
        ok.className = 'ps-m-ok'; ok.textContent = '打包下载';
        ok.onclick = async () => {
            const urls = [...new Set((ta.value.match(/https?:\/\/[^\s,]+/gi) || []).map(u => norm(origUrl(u))).filter(Boolean))];
        ...
        return new Promise((ok, no) => {
            if (typeof GM_download === 'function') {
                GM_download({
                    ...
                    onload: ok,
                    onerror(e) {
                        console.warn('[DT] GM_download 失败, 回退 fetch:', e);
    ...
    function ext(url, mime) { if (mime) { const m = { 'image/jpeg': 'jpg', 'image/png': 'png', 'image/webp': 'webp', 'image/gif': 'gif' }[mime]; if (m) return m; } const x = url.split('?')[0].match(/\.(\w{3,5})$/); if (x) return x[1].toLowerCase(); return 'jpg'; }
    ...
    function init() {
        ...
        save(SK.dock, false);
        console.log(`[DT Selector] v1.0 ready, ${selected.size} saved, ${contentImgs().length} images found`);
    }
